<div>
    ...
    <x-plantilla-admin>
        ...
        <div class=" overflow-hidden shadow-xl sm:rounded-lg ">
            <div class="flex items-center  flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 bg-white dark:bg-gray-900">
                ...
                <div class="relative mx-5">
                    ...
                    <x-input class=" pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg md:w-80 w-56 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Buscar..." type="search" wire:model.live="search"></x-input>

                </div>
                @livewire('crear-producto')
            </div>
            @if (!$productos->count())
            <h1 class="mb-4 text-4xl font-extrabold mt-6 text-center leading-none tracking-tight text-red-600 md:text-5xl lg:text-6xl dark:text-white">
                ...
                <tbody class="block md:table-row-group">
                    ...
                    <tr class=" border-b-2 md:border-none block md:table-row">
                        <td class="p-2 ml-8 md:border md:border-none block md:table-cell">
                            ...
                            <img class="w-10 h-10 rounded-full" src="{{ Storage::url($producto->imagen) }}" alt="{{ $producto->nombre }}" />
                        </td>
                        ...
                        <td class="p-2 ml-8 md:border md:border-none text-left block md:table-cell">
                            ...
                            <!-- Modal toggle -->
                            <button class="descripcion-btn font-medium text-blue-600 hover:underline ms-3" data-producto-descripcion="{{ $producto->descripcion }}">
                        ...
                        <td class="p-2 ml-8 md:border md:border-none text-left block md:table-cell">
                            ...
                            <div class="dropdown-container">
                                ...
                                <button id="dropdownActionButton" onclick="toggleDropdown('{{ $producto->id }}')" class="text-gray-900 bg-white  hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-centerdark:bg-gray-800 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700" type="button">
                                    Acción
                                ...
                                <!-- Menú desplegable -->
                                <div id="dropdownAction_{{ $producto->id }}" @class([ 'z-10 hidden bg-white  rounded-lg shadow dark:bg-gray-400' , 'dropdown-menu-last'=> $loop->last,
                                    ...
                                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownActionButton">
                                        <li>
                                            <button wire:click="edit({{ $producto->id }})" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                <p class="text-red-800 ">Editar</p>
                                            </button>
                                        </li>
                                        <li>
                                            <button wire:click="pedirConfirmacion('{{ $producto->id }}')" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
            ...
            @endif
            <!-- Descripcion modal -->
            <div id="descripcion-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed z-50 inset-0 justify-center items-center top-1/2 md:left-1/2 transform md;-translate-x-1/4 -translate-y-1/2">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                            ...
                            <button type="button" id="descripcion-modal-close-btn" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm h-8 w-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
            ...
            <x-dialog-modal wire:model='abrirModalUpdate'>
                ...
                <x-slot name="content">
                    ...
                    <div class="flex flex-row justify-between ">
                        <div class="flex flex-col w-1/2  mx-2">
                            ...
                            <div id="dropzone-imagenU" class="w-full h-64 mb-2 border-dashed border-2 border-gray-400 p-4 text-center relative" ondragover="event.preventDefault();" ondrop="handleDrop(event, 'fileInput-imagenU');">
                                @if ($form->imagenU)
                                <img src="{{ $form->imagenU->temporaryUrl() }}" class="w-full my-auto h-60 object-contain absolute inset-0" />
                                @elseif ($form->producto->imagen)
                                <img src="{{ Storage::url($form->producto->imagen) }}" class="w-full my-auto h-60 object-contain absolute inset-0" />
                                @endif
                            </div>
                        ...
                        <div class="flex flex-col w-1/2 mx-2">
                            ...
                            <div id="dropzone-imagenSFU" class="w-full h-64 mb-2 border-dashed border-2 border-gray-400 p-4 text-center relative" ondragover="event.preventDefault();" ondrop="handleDrop(event, 'fileInput-imagenSFU');">
                                @if ($form->imagenSFU)
                                <img src="{{ $form->imagenSFU->temporaryUrl() }}" class="w-full my-auto h-60 object-contain absolute inset-0" />
                                @else
                                <img src="{{ Storage::url(str_replace('.jpg', '_SF.png', $form->producto->imagen)) }}" class="w-full my-auto h-60 object-contain absolute inset-0" />
                                @endif
                            </div>
                ...
                <x-slot name="footer">
                    <div class="flex flex-row-reverse">
                        <button wire:click="update" wire:loading.attr='disabled' class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            <i class="fas fa-edit"></i> EDITAR
                        </button>
                        <button wire:click="limpiarCerrarUpdate" class="mr-2 bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
            ...
            <x-dialog-modal wire:model='abrirModalQR'>
                ...
                <x-slot name="content">
                    ...
                    <div class="flex flex-col items-center justify-center">
                        <div id="qr" style="background-color: {{ $backgroundColor }}" class="w-60 mb-2 p-10 rounded-xl text-center flex flex-col items-center justify-center overflow-hidden">
                            <h1 style="color: {{ $textColor }}" class="text-white text-3xl font-bold mb-2 break-words">
                                {{ $titulo }}
                            </h1>
                            <div class="p-2">
                                {{ QrCode::size(150)->generate(route('productos.show', $id)) }}
                            </div>
                            <p style="color: {{ $textColor }}" class="mt-2 text-sm break-words">
                                {{ $descripcion }}
                            </p>
                        </div>
                    ...
                    <input id="bg" type="color" wire:model.live="backgroundColor" value="{{ $backgroundColor }}">
                    ...
                    <x-label for="descripcion">Descripcion</x-label>
                    <x-input id="descripcion" placeholder="Descripcion..." class="w-full mb-2" wire:model.live="descripcion" />

                </x-slot>
                <x-slot name="footer">
                    <div class="flex flex-row-reverse">
                        <button id="descargar-btn" onclick="descargarqr()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" data-producto-id="{{ $id }}">
                            <i class="fas fa-save"></i> DESCARGAR
                        </button>

                        <button wire:click="cancelarQR" class="mr-2 bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
